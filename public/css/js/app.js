...
function renderAll() {
  ...
  renderNews(); renderBible();
}

function renderIntegrationDots() {
  const services = [
    {key:'google',label:'Google'}, {key:'spotify',label:'Spotify'},
    {key:'github',label:'GitHub'}, {key:'discord',label:'Discord'},
    {key:'twitch',label:'Twitch'}, {key:'duolingo',label:'Duolingo'},
    {key:'weather',label:'Weather'}, {key:'news',label:'News'}, {key:'lastfm',label:'Last.fm'},
  ];
...
function renderConnectBanner() {
  ...
  document.getElementById('connectGrid').innerHTML = services.map(s =>
  ...
  try {
    ...
    document.getElementById('weatherBody').innerHTML = `
      <div class="weather-main">
        <img src="https://openweathermap.org/img/wn/${d.icon}@2x.png" style="width:52px;height:52px" />
        <div>
...
async function loadLastFm() {
  ...
  try {
    ...
    document.getElementById('lastfmBody').innerHTML =
      ...
      d.recentTracks.slice(0,5).map(t => `
        <div class="lastfm-row">
          ...
          ${t.nowPlaying?'<span class="lastfm-now">▶ now</span>':t.date?`<span style="font-size:10px;color:var(--muted)">${relTime(t.date)}</span>`:''}
        </div>`).join('');
  } catch(e) {}
}
...
document.getElementById('refreshAllBtn').addEventListener('click', async () => {
  ...
  await loadDashboard(); loadWeather(); loadLastFm();
