...
router.get('/status', (req, res) => res.json({
  ...
  twitch: twitch.isConnected(), notion: notion.isConnected(),
  duolingo: duolingo.isConfigured(), weather: weather.isConfigured(),
  nasa: nasa.isConfigured(), news: news.isConfigured(),
  steam: steam.isConfigured(), lastfm: lastfm.isConfigured(),
}));
...
router.get('/dashboard', async (req, res) => {
  try {
    ...
    const notion_pages = db.prepare('SELECT * FROM notion_pages ORDER BY last_edited DESC LIMIT 8').all();
    ...
    res.json({ tasks, emails, calendar, github_items, discord_msgs, spotify_state, youtube,
      duolingo: duo, twitch_streams, notion_pages, news_items, bible_verse });
  } catch (e) { res.status(500).json({ error: e.message }); }
});

// Live data endpoints (called client-side)
router.get('/weather', async (req, res) => {
  try {
    const data = await weather.fetchWeather(
      req.query.lat || process.env.WEATHER_LAT || '40.7128',
      req.query.lon || process.env.WEATHER_LON || '-74.0060',
      req.query.city
    );
    res.json(data || {});
  } catch (e) { res.status(500).json({ error: e.message }); }
...
router.get('/nasa', async (req, res) => {
  try { res.json(await nasa.fetchAPOD() || {}); }
  catch (e) { res.status(500).json({ error: e.message }); }
});

router.get('/steam', async (req, res) => {
  try { res.json(await steam.fetchSteam() || {}); }
  catch (e) { res.status(500).json({ error: e.message }); }
});
...
router.post('/refresh/:provider', async (req, res) => {
  ...
  try {
    ...
    } else if (provider === 'notion') { data = await notion.fetchPages();
